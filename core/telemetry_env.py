# core/telemetry_env.py
# ============================================================
# O-RAN E2 节点 K-slice 仿真器 (gym.Env).
# 每片 slice 自带 traffic dynamics + belief engine + SLA target,
# action 是 K 维 simplex-constrained PRB allocation.
# ============================================================
...
# ---------- Slice traffic profiles (异质 per-slice 配置) ----------
...
class E2_Node_Simulator(gym.Env):
    ...
    def __init__(self, mode="proposed", K=3, tau=0.5, base_period=200,
        ...
        # Per-slice SLA threshold (可异质)
        self.tau_k = np.full(K, tau)
        self.target_latency_sla = tau

        # Obs: [global_kpm_avg, global_sla, belief_mean_1..K, entropy_1..K, tau_1..K]
        obs_dim = 2 + 3 * K
        ...
        self.tau_per = np.full(K, 0.9)  # 每 slice perception threshold
    ...
    def step(self, xapp_action):
        ...
        if self.mode == "safeslice":
            ...
            # 自适应 telemetry → RIC 侧 safety fusion → 全局 trust index → BS 侧 reflex → 融合
            ...
            # Global trust index: worst-case scalarization
            rho_k = np.array([
        ...
        else:  # vanilla_ppo, lstm_predictive 等
            final_action = self._simplex_project(raw_action)
            current_lam = np.zeros(self.K)
            gamma = 1.0
            for tc in self.telemetry_controllers:
    ...
    def _get_local_safe_action(self):
        # BS 侧 1ms 局部反应: 离 SLA 越近 throttle 越强
        margin_width = 0.15
        ...
        for k in range(self.K):
            ...
            if margin < 0:
                ...
                a_guard[k] = 0.2 + 0.75 * (1.0 - margin / margin_width)
            else:
                a_guard[k] = 0.2
    ...
    def set_tau(self, new_tau):
        # A1 接口: 调 perception threshold
        self.tau_per = np.full(self.K, new_tau)

    def set_a1_policy(self, new_sla):
        # A1 intent injection: 改 per-slice SLA + 联动 perception threshold
